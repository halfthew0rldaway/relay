...
private const val SERVICE_TYPE = "_locallink._tcp."
private const val SERVICE_NAME_PREFIX = "LocalLink-"
 ...
 *
 * ## Strategi 1 — NSD / mDNS
 * Meregister service `_locallink._tcp.` dengan IPv4 device sebagai TXT record
 ...
 * HTTP GET `/health` ke setiap IP. Device yang menjalankan server LocalLink
...
class NsdDiscovery(private val context: Context) {
    ...
    fun observeDevices(): Flow<Map<String, DiscoveredDevice>> = callbackFlow {
        ...
        val multicastLock = wifiManager.createMulticastLock("locallink_mcast_lock")
        ...
        suspend fun probeHealth(ip: String, localIp: String): DiscoveredDevice? = withContext(Dispatchers.IO) {
            ...
            try {
                ...
                try {
                    ...
                    val service = json.optString("service", "locallink").lowercase()
                    if (service != "locallink") return@withContext null
        ...
        val discoveryListener = object : NsdManager.DiscoveryListener {
            ...
            override fun onServiceFound(info: NsdServiceInfo) {
                // Harus mengandung 'locallink' dan 'tcp'
                val t = info.serviceType ?: ""
                val matchesType = t.contains("locallink", ignoreCase = true) && t.contains("tcp", ignoreCase = true)
